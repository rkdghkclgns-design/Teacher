        ...
        for (let idx = 0; idx < blueprint.length; idx++) {
            ...
            try {
                ...
                const lessonPrompt = `
                            ...
                            위 내용을 바탕으로 우리 교안 규격(1.학습 목표, 2.개요(이 차시 타임테이블 포함), 3.핵심 내용, 4.실습/예제, 5.요약) 순서에 맞게 마크다운 교안을 작성하세요.
...
function buildTaskContext(taskScope, additionalContext = "") {
    ...
    function buildFullRules(keyConcepts) {
        ...
        rules += '\n\n[자체 검증 체크리스트]\n' +
            ...
            '4. 분량 기준: 기본 학습(학습목표+개요+핵심내용) 영역은 공백과 서식(마크다운 기호, HTML 태그)을 제외한 순수 텍스트 기준 최소 7,000자를 충족하는가? 기본 실습·심화는 필요한 항목만 포함되면 분량 무관.\n' +
            '5. 서식 준수: 헤딩 위계, 아이콘, 간격, 이미지 태그 규칙을 모두 지켰는가?\n' +
            '6. 구조 연결성: 학습목표→개요(타임테이블)→핵심→실습→심화의 흐름이 자연스러운가? 학습 목표가 최상단, 그다음 개요(타임테이블)인가?\n' +
            '7. 용어 일관성: 과정/교과/차시/레벨테스트/기본실습 등 glossary 정의와 일치하는가?\n' +
            '8. 서식 워크플로우 준수: 파싱→타이포매핑→아이콘주입→들여쓰기→간격적용→톤적용 순서를 따랐는가?';

        // 메타 라벨 누출 방지 (요청#2): 작업용 표시가 학생 교안 본문에 노출되지 않도록
        rules += '\n\n[출력 금지 — 메타 라벨 누출 방지]\n' +
            '- 다음과 같은 내부 작업 라벨을 본문 헤딩이나 텍스트로 절대 출력하지 마세요: "[상세 교안]", "[현재 작업: ...]", "[기본 학습]", "[탭 범위 제한]", "교육용 일러스트", "[내부 프로세스]" 등.\n' +
            '- 이들은 집필 지시용 표시일 뿐이며, 학생이 읽는 교안 본문에 그대로 노출되어서는 안 됩니다.\n' +
            '- 교안은 실제 학습 내용(# 제목 → ## 섹션 → 본문)으로 바로 시작하세요. 작업 설명·메타 헤더·따옴표로 감싼 라벨로 문서를 시작하지 마세요.';
    ...
    switch (taskScope) {
        ...
        case 'module':
            ...
            if (additionalContext.uploadedMdContent) {
                ...
                userPrompt = `${thinkingPrefix}[현재 작업: 사용자 제공 문서 기반 교안 재구성]\n모듈명: ${additionalContext.title}\n설명: ${additionalContext.description}\n\n[사용자 제공 원본 마크다운 데이터]\n${safeMd}\n\n위 원본 데이터를 철저히 분석하여, 제공된 내용을 바탕으로 우리 교안 규격(1.학습 목표, 2.개요(이 차시 타임테이블 포함), 3.핵심 내용, 4.실습/예제, 5.심화) 순서에 맞게 다듬고 재구성해 주세요. 원본의 핵심 내용과 의도를 절대 누락하지 말고 최우선으로 반영해야 합니다.`;
        ...
        case 'tab_content': {
            ...
            function buildTabRules(keyConcepts) {
                ...
                if (additionalContext.tabId === 'basicLearn' || additionalContext.tabId === 'advLearn') {
                    rules += '\n\n[탭 범위 제한 — 절대 준수]\n' +
                        ...
                        '- 각 ## 섹션은 [## 제목] → [강사 스크립트 박스] → [본문] 순서로 작성하세요. ## 제목 바로 다음 줄(본문 시작 전, 본문 서두)에 <div class="instructor-callout"> 강사 스크립트를 먼저 삽입합니다. (박스를 본문 뒤에 두지 마세요.)\n' +
                        '- 강사 스크립트는 위 강사 Callout 규칙의 4개 항목(⏰ 예상 소요 시간 / 💡 해당 세션에서 수강생이 인지하여야 할 항목 / 💡 핵심 강조 항목 / 💡 활용 교보재)을 그 순서대로 모두 포함하세요. 쓸 내용이 없는 항목은 생략하지 말고 "-"로 표기하세요.\n' +
                        '- 단, ## 🎓 교강사 가이드 (강사 전용) "섹션"은 학습 탭에 포함하지 마세요. (이 섹션은 기본실습/심화실습 탭에서 작성)';
                }

                // 실습 탭 전용: 강사뷰(instructor-callout) 박스 — 학습 탭과 동일한 강사뷰를 실습 탭에도 적용
                if (additionalContext.tabId === 'basicPrac' || additionalContext.tabId === 'advPrac') {
                    rules += '\n\n[강사뷰 규칙 — 실습 탭에서도 반드시 포함]\n' +
                        '- "실습 목표 및 개요" 섹션과 각 실습 과제 섹션은 [제목] → [강사뷰 박스] → [본문] 순서로 작성하세요. 즉 각 섹션 제목 바로 다음 줄(본문 시작 전, 본문 서두)에 <div class="instructor-callout"> ... </div> 강사뷰(강사 스크립트)를 먼저 삽입합니다. (박스를 본문 뒤에 두지 마세요.)\n' +
                        '- 강사뷰는 위 강사 Callout 규칙의 4개 항목을 그 순서대로 모두 포함하세요: ⏰ 예상 소요 시간 / 💡 해당 세션에서 수강생이 인지하여야 할 항목(실습 목표·달성 기준) / 💡 핵심 강조 항목(진행 요령·시간 관리·학생이 자주 막히는 지점과 대응) / 💡 활용 교보재(예제 파일·실습 도구·참고 자료). 쓸 내용이 없는 항목은 생략하지 말고 "-"로 표기하세요.\n' +
                        '- 강사뷰는 강사 전용 안내이며, 학생용 실습 과제 본문과 명확히 구분해 작성하세요.\n' +
                        '- 기존 ## 🎓 교강사 가이드 (강사 전용) 섹션도 그대로 유지하세요. (강사뷰 박스는 이와 별개로 각 섹션마다 추가)';
                }
                ...
                rules += '\n\n[출력 금지 — 메타 라벨 누출 방지]\n' +
                    '- 다음과 같은 내부 작업 라벨을 본문 헤딩이나 텍스트로 절대 출력하지 마세요: "[상세 교안]", "[현재 작업: ...]", "[기본 학습]", "[탭 범위 제한]", "교육용 일러스트", "[내부 프로세스]" 등.\n' +
                    '- 이들은 집필 지시용 표시일 뿐이며, 학생이 읽는 교안 본문에 그대로 노출되어서는 안 됩니다.\n' +
                    '- 교안은 실제 학습 내용(## 섹션 → 본문)으로 바로 시작하세요. 작업 설명·메타 헤더·따옴표로 감싼 라벨로 문서를 시작하지 마세요.';
            ...
            // 분할 생성 지시 — 긴 탭(특히 기본 학습 480분)이 중간에 잘리는 것을 방지
            systemInstruction += '\n\n[분량 관리]\n' +
                '- 탭 내용을 한 번에 작성하세요. 단, 출력이 길어져 중간에 잘릴 경우를 대비하여 다음 규칙을 따르세요.\n' +
                '- 모든 섹션을 완전히 작성한 경우: 문서의 맨 마지막에 <!-- END -->를 붙이세요.\n' +
                '- 출력 한도에 도달하여 미완성인 경우: 자연스러운 문단/섹션 끝에서 멈추고 <!-- CONTINUE -->를 붙이세요. 문장 중간에서 끊지 마세요.';

            const tabPrompts = {
                basicLearn: '이 교안의 "기본 학습" 영역을 작성하세요. 반드시 [1.학습 목표] → [2.개요(이 차시 타임테이블 포함)] → [3.핵심 내용(개념 정의, 이론 설명, 예시, 사례 분석)] 순서로 구성합니다. 학습 목표가 교안 최상단에 오고, 그 다음 개요(타임테이블)가 옵니다. 실습 과제, 평가 루브릭, 별도 강사 가이드 섹션은 다른 탭에서 작성하므로 여기서는 포함하지 마세요. 초보자도 이해할 수 있는 수준으로 단계적으로 설명하세요.\n' +
                    '[시간 배분 — 이 탭 하나가 곧 480분(8시간) 본과정입니다]\n' +
                    '이 "기본 학습" 탭 하나가 480분(8시간) 분량의 완결된 본 교안입니다. 다른 탭(실습/심화/평가)에 의존하지 말고, 이 탭만으로 하루 8시간 수업을 끝까지 진행할 수 있도록 작성하세요.\n' +
                    '각 ## 섹션 시작에 ⏱️ 예상 소요 시간을 명시하고, 모든 섹션의 합계가 480분(±30분)이 되도록 구성하세요.\n' +
                    '시간을 채우기 위해 각 개념을 충분히 깊고 상세하게, 다양한 예시·비유·단계적 설명으로 풀어쓰세요. 개념을 나열만 하지 말고 "왜 그런지"까지 설명하세요.\n\n' +
                    '[분량 규칙 — 절대 준수]\n' +
                    '- 순수 텍스트(마크다운 서식 제외) 최소 40,000자 이상.\n' +
                    '- 마크다운 줄 수 기준 최소 900줄 이상.\n' +
                    '- ## 대주제 최소 8개, ### 소주제 최소 16개.\n' +
                    '- 각 소주제마다 Key Concept 불릿 최소 6개, 구체적 사례(게임명 명시) 최소 2개, 본문 설명 단락 최소 3개.\n' +
                    '- 표(Table) 최소 6개, mermaid 다이어그램 최소 3개.\n' +
                    '- 각 ## 섹션은 제목 직후(본문 시작 전, 본문 서두)에 강사 스크립트(instructor-callout)를 4항목 구조로 먼저 넣고, 그 아래에 본문을 작성.\n' +
                    '- 짧은 교안은 불합격 처리됩니다. 480분을 채울 만큼 매우 상세하게 작성하세요.\n\n' +
                    '[분할 출력 규칙 — 절대 준수]\n' +
                    '- 내용이 매우 길어 한 번에 다 출력하지 못할 수 있습니다.\n' +
                    '- 모든 섹션을 끝까지 완성했다면 문서 맨 끝에 <!-- END -->를 붙이세요.\n' +
                    '- 출력 한도에 도달하면 자연스러운 문단/섹션 끝에서 멈추고 <!-- CONTINUE -->를 붙이세요. 문장 중간에서 끊지 마세요.',
                basicPrac: '이 교안의 "기본 실습" 영역을 작성하세요. 학생이 핵심 내용을 직접 체험하는 실습 과제를 설계하세요.\n' +
                    '[시간 배분 — 이 탭: 90~120분]\n' +
                    '이 탭은 "기본 학습" 본과정을 보완하는 독립 기본 실습 모듈로, 90~120분 분량입니다.\n' +
                    '각 ## 섹션의 ⏱️ 예상 소요 시간을 명시하고, 합계가 90~120분이 되게 하세요.\n\n' +
                    '[분량 규칙 — 절대 준수]\n' +
                    '- 순수 텍스트(마크다운 서식 제외) 최소 8,000자 이상.\n' +
                    '- 마크다운 줄 수 기준 최소 250줄 이상.\n' +
                    '- ## 대주제(실습 파트) 최소 3개.\n' +
                    '- 짧은 교안은 불합격 처리됩니다.\n\n' +
                    '[필수 포함 항목]\n' +
                    '- 실습 목표 및 개요 (기본학습에서 배운 핵심 개념과 연결)\n' +
                    '- 실습 과제 최소 3개 (각 과제에 아래 항목 모두 포함):\n' +
                    '  * 과제 제목, 과제 설명 (상세하게)\n' +
                    '  * 단계별 수행 절차 (Step-by-Step, 최소 5단계)\n' +
                    '  * ⏱️ 예상 소요시간, 🎚️ 난이도(초급)\n' +
                    '  * 예시 결과물 또는 기대 산출물 설명\n' +
                    '  * 평가 기준 (체크리스트 또는 루브릭)\n' +
                    '- 실습 중 자주 하는 실수 및 해결 가이드\n' +
                    '- ## 🎓 교강사 가이드 (강사 전용): 실습 진행 요령, 시간 관리 팁, 피드백 포인트',
                advLearn: '이 교안의 "심화 학습" 영역을 작성하세요. 기본 학습의 개념을 더 깊이 파고들어 고급 이론, 업계 사례, 실무적 분석을 제공하세요. 실습, 평가, 강사 가이드는 다른 탭에서 작성하므로 여기서는 절대 포함하지 마세요.\n' +
                    '[시간 배분 — 이 탭: 90~120분]\n' +
                    '이 탭은 "기본 학습" 본과정 이후 진행하는 독립 심화 학습 모듈로, 90~120분 분량입니다.\n' +
                    '각 ## 섹션의 ⏱️ 예상 소요 시간을 명시하고, 합계가 90~120분이 되게 하세요.\n\n' +
                    '[분량 규칙 — 절대 준수]\n' +
                    ...
                    '- 짧은 교안은 불합격 처리됩니다.',
                advPrac: '이 교안의 "심화 실습" 영역을 작성하세요. 도전적이고 창의적인 프로젝트형 과제를 설계하세요.\n' +
                    '[시간 배분 — 이 탭: 90~120분]\n' +
                    '이 탭은 "기본 학습" 본과정을 보완하는 독립 심화 실습 모듈로, 90~120분 분량입니다.\n' +
                    '각 ## 섹션의 ⏱️ 예상 소요 시간을 명시하고, 합계가 90~120분이 되게 하세요.\n\n' +
                    '[분량 규칙 — 절대 준수]\n' +
                    '- 순수 텍스트(마크다운 서식 제외) 최소 8,000자 이상.\n' +
                    '- 마크다운 줄 수 기준 최소 250줄 이상.\n' +
                    ...
                    '- 짧은 교안은 불합격 처리됩니다.\n\n' +
                    '[필수 포함 항목]\n' +
                    '- 심화 실습 목표 및 개요 (심화학습에서 배운 고급 개념과 연결)\n' +
                    '- 프로젝트형 실습 과제 최소 2개 (각 과제에 아래 항목 모두 포함):\n' +
                    '  * 프로젝트 제목, 시나리오 설명 (실무 상황 기반)\n' +
                    '  * 단계별 수행 절차 (Step-by-Step, 최소 7단계)\n' +
                    '  * ⏱️ 예상 소요시간, 🎚️ 난이도(중급~고급)\n' +
                    '  * 예시 결과물 또는 기대 산출물 설명\n' +
                    '  * 평가 기준 (100점 만점 루브릭: S/A/B/C/F)\n' +
                    '- 도전 과제 또는 보너스 미션 1개\n' +
                    '- 팀 프로젝트 진행 시 역할 분배 가이드\n' +
                    '- ## 🎓 교강사 가이드 (강사 전용): 프로젝트 진행 요령, 발표/피드백 운영 가이드, 채점 기준 상세',
                assessment: '이 교안의 "학습 이해도 점검" 영역을 작성하세요.\n' +
                    '[시간 배분 — 이 탭: 30~60분]\n' +
                    '이 탭은 "기본 학습" 본과정의 이해도를 점검하는 독립 평가 모듈로, 30~60분 분량입니다.\n\n' +
                    '[형식 규칙 — 반드시 준수]\n' +
                    ...
                    '   - [필수] 채점 기준표는 반드시 인라인 스타일 HTML <table>로 출력하세요. 마크다운 파이프 표(| ... |)나 줄글로 작성하지 마세요.\n' +
                    '   - 표 구조: 1열 "등급"(S/A/B/C/F), 2열 "점수 범위"(예: 90~100점), 3열 "평가 기준 설명". 5개 등급을 각각 한 행으로 작성.\n' +
                    '   - 표 양식 예시: <table style="border-collapse: collapse; width: 100%;"><thead><tr><th style="border: 1px solid black; padding: 10px; background-color: #f2f2f2; text-align: center; width: 100px;">등급</th><th style="border: 1px solid black; padding: 10px; background-color: #f2f2f2; text-align: center; width: 140px;">점수 범위</th><th style="border: 1px solid black; padding: 10px; background-color: #f2f2f2; text-align: center;">평가 기준</th></tr></thead><tbody><tr><td style="border: 1px solid black; padding: 10px; font-weight: bold; text-align: center;">S</td><td style="border: 1px solid black; padding: 10px; text-align: center;">90~100점</td><td style="border: 1px solid black; padding: 10px; text-align: left;">설명...</td></tr></tbody></table>\n\n' +
                    '[단락 구분 규칙 — 절대 준수]\n' +
        ...
        // ─── 탭 분할 생성 이어쓰기 (긴 탭, 특히 기본 학습 480분이 잘리지 않도록) ───
        case 'tab_content_continue': {
            // buildTabRules는 tab_content 케이스 블록 내부 스코프라 여기서 접근 불가.
            // 이어쓰기는 1청크에서 형성된 문체·구조를 그대로 잇는 것이 핵심이므로 자체 규칙으로 구성.
            systemInstruction = `${CONTEXT_CORE.personas.instructor}\n[작성 규칙]\n` +
                '- 반드시 한국어로 작성하세요. 영어로 작성하지 마세요.\n' +
                '- 마크다운 구조(##, ###, 표, mermaid, 불릿)와 이전 청크의 문체·깊이를 그대로 유지하세요.\n' +
                '- 이 탭의 성격에 맞지 않는 섹션을 새로 만들지 마세요. (학습 탭에 채점 루브릭·퀴즈를 추가하지 말 것)\n' +
                '- 각 ## 섹션은 [제목] → [강사 스크립트 박스] → [본문] 순서로 작성하세요. ## 제목 바로 다음 줄(본문 시작 전)에 <div class="instructor-callout">를 먼저 넣되, 다음 4개 항목을 이 순서로 포함하세요: ⏰ **예상 소요 시간:** ○○분(실제 숫자) / 💡 **해당 세션에서 수강생이 인지하여야 할 항목** / 💡 **핵심 강조 항목** / 💡 **활용 교보재**. 쓸 내용이 없는 항목은 제목 줄 바로 아래에 "-" 한 줄만. 이미지 태그 형식도 이전 청크와 동일하게.';
            systemInstruction += '\n\n[이어쓰기 규칙]\n' +
                '- 아래 "이전 작성 내용"의 마지막 지점부터 이어서 작성하세요. 절대 처음부터 다시 쓰지 마세요.\n' +
                '- 이미 작성된 내용을 반복하지 마세요. 중복 헤딩이나 중복 문단을 생성하면 안 됩니다.\n' +
                '- 이전 내용의 마지막 헤딩/문단을 확인하고, 그 다음 섹션부터 자연스럽게 연결하세요.\n' +
                '- 아직 작성되지 않은 남은 섹션을 모두 완성하세요.\n' +
                '- 모든 내용을 마쳤으면 문서 끝에 <!-- END -->를 붙이세요. 아직 더 남았으면 자연스러운 문단/섹션 끝에서 멈추고 <!-- CONTINUE -->를 붙이세요. 문장 중간에서 끊지 마세요.';

            userPrompt = `[현재 작업: 탭 이어쓰기 (${additionalContext.chunkIndex}번째 연속) — ${additionalContext.tabLabel}]\n모듈명: ${additionalContext.title}\n설명: ${additionalContext.description}\n\n[이전 작성 내용 — 여기서부터 이어쓰세요. 반복 금지]\n${additionalContext.previousContent}`;
            break;
        }
