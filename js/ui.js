    ...
    switch (taskScope) {
        ...
        case 'tab_content': {
            ...
            function buildTabRules(keyConcepts) {
                ...
                if (additionalContext.tabId === 'basicPrac' || additionalContext.tabId === 'advPrac') {
                    rules += '\n\n[강사뷰 규칙 — 실습 탭에서도 반드시 포함]\n' +
                        '- "실습 목표 및 개요" 섹션 직후, 그리고 각 실습 과제 섹션 직후에 <div class="instructor-callout"> ... </div> 강사뷰(강사 스크립트)를 반드시 삽입하세요.\n' +
            ...
            const tabPrompts = {
                basicLearn: '이 교안의 "기본 학습" 영역을 작성하세요. 개요, 학습 목표, 핵심 내용(개념 정의, 이론 설명, 예시, 사례 분석)만 포함합니다. 실습 과제, 평가 루브릭, 별도 강사 가이드 섹션은 다른 탭에서 작성하므로 여기서는 포함하지 마세요. 초보자도 이해할 수 있는 수준으로 단계적으로 설명하세요.\n' +
                    ...
                    '- 각 ## 섹션 직후 강사 스크립트(instructor-callout)를 충분히 길게 포함.\n' +
        ...
        case 'tab_content_continue': {
            ...
            systemInstruction = `${CONTEXT_CORE.personas.instructor}\n[작성 규칙]\n` +
                ...
                '- 각 ## 섹션 직후 강사 스크립트(<div class="instructor-callout">)를 반드시 넣되, 다음 4개 항목을 이 순서로 포함하세요: ⏰ **예상 소요 시간:** ○○분(실제 숫자) / 💡 **해당 세션에서 수강생이 인지하여야 할 항목** / 💡 **핵심 강조 항목** / 💡 **활용 교보재**. 쓸 내용이 없는 항목은 제목 줄 바로 아래에 "-" 한 줄만. 이미지 태그 형식도 이전 청크와 동일하게.';
            systemInstruction += '\n\n[이어쓰기 규칙]\n' +
