...
function wrapOrphanedInstructorMarkers(md) {
    ...
    const INSTRUCTOR_MARKERS = new RegExp(
        ...
        '(?:' +
            ...
            '|도입\\s*멘트' +
            '|핵심\\s*설명(?:\\s*대본)?' +
            '|꼭\\s*짚어(?:야|야\\s*할\\s*핵심)?' +    // 꼭 짚어야 할 핵심
...
const CONTEXT_CORE = {
    ...
    rules: {
        ...
        instructor_callout:
            ...
            '💡 **꼭 짚어야 할 핵심** (반드시 강조할 포인트 — 학생이 시험에 나올 수 있는 핵심)\n' +
            '- 2~3가지 불릿 포인트\n' +
            '- 실무에서 자주 쓰이는 내용을 구체적으로\n\n' +

            '💼 **활용 예시** (실제 현장·게임에서 적용된 사례 — 이 섹션 개념의 활용)\n' +
            '- 2~3가지 구체 예시 (게임명·회사·상황 명시)\n' +
            '- 각 예시에 "어떻게 활용되었는지" 1~2문장 설명\n' +
            '- 가능하면 학생이 직접 플레이/조사 가능한 게임 위주로\n\n' +

            '⚠️ **자주 헷갈리는 포인트** (학생이 오해하기 쉬운 부분)\n' +
            '- 흔한 오해를 먼저 언급하고 올바른 이해를 제시\n' +
            '- (예: "많은 분들이 ~라고 오해하시는데, 실제로는 ~입니다.")\n\n' +

            '❓ **권장 질문** (학생 참여를 이끌어낼 개방형 질문)\n' +
            '- 권장 질문 2~3개 (예: "여러분은 ~를 해본 경험이 있나요?")\n' +
            '- 각 질문에 예상 답변과 후속 연결 시나리오 제시\n' +
            '- (예: "학생이 ~라고 답하면 → ~로 연결")\n\n' +
